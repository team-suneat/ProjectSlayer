...
{
    /// <summary>
    /// ScriptableDataManager의 몬스터 스탯 설정 관련 기능
    /// </summary>
    public partial class ScriptableDataManager
    {
        ...
        private void CheckValidMonsterStatConfigOnLoadAssets()
        {
            ...
            {
                Log.Warning(LogTags.ScriptableData, "몬스터 스탯 설정 에셋이 설정되지 않았습니다.");
                return;
            }

            if (_monsterStatConfigAsset.BaseHealth <= 0)
            {
                Log.Warning(LogTags.ScriptableData, "몬스터 스탯 설정의 기본 체력이 0 이하입니다.");
            }

            if (_monsterStatConfigAsset.BaseAttack <= 0)
            {
                Log.Warning(LogTags.ScriptableData, "몬스터 스탯 설정의 기본 공격력이 0 이하입니다.");
            }

            if (_monsterStatConfigAsset.NormalStageHealthGrowthRate <= 1.0f)
            {
                Log.Warning(LogTags.ScriptableData, "몬스터 스탯 설정의 일반 스테이지 체력 증가 배율이 1.0 이하입니다.");
            }

            if (_monsterStatConfigAsset.NormalStageAttackGrowthRate <= 1.0f)
            {
                Log.Warning(LogTags.ScriptableData, "몬스터 스탯 설정의 일반 스테이지 공격력 증가 배율이 1.0 이하입니다.");
            }

            if (_monsterStatConfigAsset.BossStageHealthGrowthRate <= 1.0f)
            {
                Log.Warning(LogTags.ScriptableData, "몬스터 스탯 설정의 보스 스테이지 체력 증가 배율이 1.0 이하입니다.");
            }

            if (_monsterStatConfigAsset.BossStageAttackGrowthRate <= 1.0f)
            {
                Log.Warning(LogTags.ScriptableData, "몬스터 스탯 설정의 보스 스테이지 공격력 증가 배율이 1.0 이하입니다.");
            }
#endif
        }
