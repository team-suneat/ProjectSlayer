...
namespace TeamSuneat
{
    ...
    public static class MathEx
    {
        ...
        public static void Spring(ref float currentValue, float targetValue, ref float velocity, float damping, float frequency, float speed, float deltaTime)
        {
            ...
            velocity = MathEx.Lerp(initialVelocity, velocity, speed, Time.deltaTime);
        ...
        public static void Spring(ref Vector2 currentValue, Vector2 targetValue, ref Vector2 velocity, float damping, float frequency, float speed, float deltaTime)
        {
            ...
            velocity.x = MathEx.Lerp(initialVelocity.x, velocity.x, speed, Time.deltaTime);
            velocity.y = MathEx.Lerp(initialVelocity.y, velocity.y, speed, Time.deltaTime);
            currentValue += deltaTime * velocity;
        }
        ...
        public static void Spring(ref Vector3 currentValue, Vector3 targetValue, ref Vector3 velocity, float damping, float frequency, float speed, float deltaTime)
        {
            ...
            velocity.x = MathEx.Lerp(initialVelocity.x, velocity.x, speed, Time.deltaTime);
            velocity.y = MathEx.Lerp(initialVelocity.y, velocity.y, speed, Time.deltaTime);
            velocity.z = MathEx.Lerp(initialVelocity.z, velocity.z, speed, Time.deltaTime);
            currentValue += deltaTime * velocity;
        }
        ...
        public static void Spring(ref Vector4 currentValue, Vector4 targetValue, ref Vector4 velocity, float damping, float frequency, float speed, float deltaTime)
        {
            ...
            velocity.y = MathEx.Lerp(initialVelocity.y, velocity.y, speed, Time.deltaTime);
            velocity.z = MathEx.Lerp(initialVelocity.z, velocity.z, speed, Time.deltaTime);
            velocity.w = MathEx.Lerp(initialVelocity.w, velocity.w, speed, Time.deltaTime);
            currentValue += deltaTime * velocity;
        }
        ...
        public static Vector3 ProjectPointOnLine(Vector3 point, Vector3 lineStart, Vector3 lineEnd)
        {
            ...
            float dotProduct = Vector3.Dot(point - lineStart, direction);
            float clampedDotProduct = Mathf.Clamp(dotProduct, 0f, magnitude);

            return lineStart + direction * clampedDotProduct;
        ...
        /// <summary>
        /// 구간 [A,B]의 값 x를 구간 [C,D]의 비례 값으로 다시 매핑합니다.
        /// </summary>
        /// <param name="x">다시 매핑할 값입니다.</param>
        /// <param name="A">x 값을 포함하는 간격 [A,B]의 최소 경계(bound)</param>
        /// <param name="B">x 값을 포함하는 간격 [A,B]의 최대 경계(bound)</param>
        /// <param name="C">목표 간격 [C,D]의 최소 경계(bound)</param>
        /// <param name="D">대상 간격 [C,D]의 최대 경계(bound)</param>
        ...
        /// <summary>
        /// 최소 각도와 최대 각도 사이의 매개변수로 각도를 고정합니다(모든 각도는 도 단위로 표시됨).
        /// </summary>
        ...
        /// <summary>
        /// 매개변수에 전달된 값을 매개변수 배열에서 가장 가까운 값으로 반올림합니다.
        /// </summary>
        /// <param name="value"></param>
        ...
        public static int Quotient(float baseValue, float quotientByCount)
        {
            double quotient = baseValue / quotientByCount;
            double value = Math.Truncate(quotient);

            return (int)value;
        }

        public static int QuotientCeiling(float baseValue, float quotientByCount)
        {
            double quotient = baseValue / quotientByCount;
            double value = Math.Ceiling(quotient);

            return (int)value;
        }

        public static float RoundSecondNumber(double value)
        {
            double roundValue = Math.Round(value, 3);
            double resultValue = Math.Round(roundValue, 2);

            return (float)resultValue;
        }

        public static int IntTruncate(int value, int digit)
        {
            double temp = Mathf.Pow(10.0f, digit);

            return (int)(Math.Truncate(value * temp) / temp);
        }

        public static Vector3 BetweenPoint(Vector3 firstValue, Vector3 secondValue, float midPoint)
        {
            return firstValue + (secondValue - firstValue) * midPoint;
        }
