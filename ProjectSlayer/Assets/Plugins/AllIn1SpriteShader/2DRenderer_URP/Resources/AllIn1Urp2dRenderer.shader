    ...
    SubShader
    {
        ...
        Pass //Main Light Shapes pass
        {
            ...
            v2f CombinedShapeLightVertex(Attributes v)
            {
				...
				이 경고는 CombinedShapeLightFragment 함수에서 발생합니다. 
				문제가 되는 라인은 벡터 타입의 변환 과정에서 암묵적인 잘림이 발생할 수 있는 부분입니다. 
				이 경고를 해결하기 위해서는 명시적인 타입 변환을 사용하여 불필요한 데이터 손실을 방지해야 합니다.
				특히 문제가 되는 부분은 벡터 타입의 변환입니다. 

				여기서 o.vertex는 float4 타입이고, ComputeScreenPos 함수의 결과는 float4입니다. 
				ComputeScreenPos 함수가 반환하는 float4 타입의 값을 xy로 잘라서 float2 타입의 o.lightingUV에 대입하고 있습니다.
				이를 명시적으로 변환하여 경고를 제거할 수 있습니다. 
				ComputeScreenPos 함수의 반환 값을 float4로 받아서, float2로 변환하는 과정을 명시적으로 수행합니다.
				
				*/
				

				float4 clipVertex = o.vertex / o.vertex.w;
				float4 screenPos = ComputeScreenPos(clipVertex);
				o.lightingUV = float2(screenPos.x, screenPos.y);

				/*

				이렇게 명시적으로 타입 변환을 하면 경고를 제거할 수 있습니다. 
				이는 암묵적인 잘림(truncation)을 피하고, 코드의 명확성을 높이는 데 도움이 됩니다.
